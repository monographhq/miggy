...
// src/bar.js
function bar_default(row2, project2, phase) {
  render_default(row2, `
      <div class="mig-bar" style="flex: 0 0 ${phase.width}px;}">
        <div class="mig-money">
          <div class="mig-money-fill" style="background:${project2.color};width:${phase.money}%"></div>
        </div>
        <div class="mig-time" style="background:${project2.color}">
...
// src/row.js
function row_default(parent, project2) {
  const row2 = document.createElement("div");
  row2.classList.add("mig-row");
  row2.style.left = project2.left + "px";
  parent.appendChild(row2);
  project2.phases.map((phase) => {
    bar_default(row2, project2, phase);
  });
}
